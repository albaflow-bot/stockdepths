...
**구현(Δ) — 뉴스 기사 피드 (`GET /api/news`)**: 위 LLM brief 와 별개로, 사용자가 *실제 기사*를 바로 볼 수 있는 피드를 추가.
- **소스/게이트**: Google News RSS 검색(키 불필요, `q`=종목명 또는 시장 키워드, `market`=US|KR) → item `source` 를 **신뢰 언론사 화이트리스트로 필터**(찌라시·블로그·커뮤니티 차단). 본문 미수집(저작권) — 헤드라인+출처+시각+링크만, 탭 시 원문을 **앱 내부 웹뷰**(상단 ‹ 뒤로 → 원래 화면 복귀)로 연다 — 외부 브라우저 ✗(사용자 요청).
- **배치 위치(환원 정합)**: 종목 상세 시트(`관련 뉴스` — 타이밍 신호 아래 *맥락*) + 투데이(`시장 속보` — 시장 브리핑 맥락). 정보 나열로 끝내지 않고 타이밍 신호가 있는 표면 안에 보조로 배치.
- **degrade**: 보조 정보라 실패/0건은 한 줄 안내(`관련 검증 뉴스가 아직 없어요`)로만 — 화면을 막지 않음.
