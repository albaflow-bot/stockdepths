...
- **소스/게이트**: Google News RSS 검색(키 불필요, `q`=종목명 또는 시장 키워드, `market`=US|KR) → item `source` 를 **신뢰 언론사 화이트리스트로 필터**(찌라시·블로그·커뮤니티 차단). 본문 미수집(저작권) — 헤드라인+출처+시각+링크만, 탭 시 원문을 외부 브라우저로 연다.
